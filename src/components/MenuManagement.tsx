...
import { useEffect, useState } from 'react';
import { Plus, Pencil, Trash2, ChevronDown, ChevronRight, Loader, GripVertical } from 'lucide-react';
import {
  getAllCategories,
  addCategory,
  updateCategory,
...
const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
  ...
  newCategories.splice(insertAt, 0, dragged);
  ...
  await fetch(`${API_BASE_URL}/categories/reorder`, {
    ...
    body: JSON.stringify({ categories: newCategories.map((c, i) => ({ id: c.id, order: i })) })
  });
};
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-500">
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-8 animate-fade-in bg-white p-5 rounded-md">
          ...
          <p className="text-blue-800 text-sm">Manage your restaurant menu structure with ease • Drag categories to reorder</p>
        </div>
            ...
            <div className="divide-y divide-gray-200">
              {categories.map((category, idx) => (
                <div
                  ...
                  className={`relative transition-all duration-300 bg-white rounded-lg shadow-sm ${dragOverIndex === idx
                    ? 'ring-4 ring-blue-400 ring-opacity-50 bg-blue-50'
                    : ''
                    } ${draggedIndex === idx ? 'opacity-50 scale-98' : ''}`}
                >
                  <div className="bg-gradient-to-r from-slate-50 to-blue-50 px-6 py-4 hover:from-slate-100 hover:to-blue-100 transition-colors duration-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
