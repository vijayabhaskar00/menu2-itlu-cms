...
export default function MenuItem({ item }: MenuItemProps) {
  ...
  const isSeasonal = item.status === 'seasonal';
  ...
  return (
    <div 
      className={`
        ...
        ${isUnavailable ? 'opacity-75 hover:scale-100' : ''}
        ${isComingSoon ? 'opacity-90 hover:scale-102' : ''}
        ${isSeasonal ? 'ring-2 ring-orange-400 ring-offset-2' : ''}
      ...
      <div className="relative h-48 sm:h-56 md:h-60 bg-gradient-to-br from-yellow-100 to-green-50 overflow-hidden flex-shrink-0">
        ...
        <Image
          ...
          className={`object-cover w-full h-full transition-all duration-500 hover:scale-110 ${
            isUnavailable ? 'grayscale brightness-75' : ''
          } ${isComingSoon ? 'blur-[2px] brightness-90' : ''} ${isSeasonal ? 'sepia-[0.15] saturate-110' : ''} ${imageLoading ? 'opacity-0' : 'opacity-100'}`}
          priority={false}
        ...
        {isComingSoon && !imageLoading && (
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/60 to-indigo-900/60 flex items-center justify-center backdrop-blur-[1px]">
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-5 py-2 rounded-lg font-bold text-base shadow-lg animate-pulse">
             COMING SOON
            </div>
          </div>
        )}
        {isSeasonal && !imageLoading && (
          <div className="absolute top-3 right-3">
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 text-white px-3 py-1.5 rounded-full font-bold text-xs shadow-lg flex items-center gap-1">
              🍂 SEASONAL
            </div>
          </div>
        )}
      ...
      <div className="bg-gradient-to-b from-slate-800 to-slate-900 text-white p-4 md:p-5 flex flex-col flex-grow justify-between">
        ...
        <div className="flex justify-between items-center mt-4 pt-3 border-t border-slate-700">
          ...
          {item.status === 'coming-soon' && (
            <span className="text-xs md:text-sm bg-gradient-to-r from-purple-500 to-indigo-600 text-white px-3 py-1 rounded-full font-semibold shadow-md">
              Coming Soon
            </span>
          )}
          {item.status === 'seasonal' && (
            <span className="text-xs md:text-sm bg-gradient-to-r from-orange-500 to-amber-500 text-white px-3 py-1 rounded-full font-semibold shadow-md">
              Seasonal
            </span>
          )}
        </div>
